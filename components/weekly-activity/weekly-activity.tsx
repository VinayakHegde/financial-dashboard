    ...
    return (
      <div className="bg-white border border-gray-300 rounded-md shadow-md flex flex-col gap-2 pb-2 ">
        <div className='bg-gray-100 border-b border-b-blue-200 p-2'>
          <Typography type='body' weight='semibold' size='md'>{label}</Typography>
        </div>
        {payload.map((entry, index: number) => (
          <div key={`item-${index}`} className={`px-2 flex items-center ${entry.name === 'deposit' ? 'text-info' : 'text-gray-1000'}`}>
            <Typography type='body' weight='semibold' size='sm' color='inherit'>{`${entry.name}:`}</Typography>
            <Typography type='body' weight='semibold' size='sm' color='inherit'>{toCurrency(Number(entry.value))}</Typography>
          </div>
        ))}
      </div>
    );
    ...
    <div className="flex flex-row-reverse gap-4 justify-start items-center pb-5">
      {payload.map((entry, index: number) => (
        <div key={index} className="flex items-center gap-2 capitalize">
          <div className={`w-4 h-4 rounded-full ${entry.value === 'withdrawal' ? 'bg-gray-1000' : 'bg-info'}`} />
          <Typography type='body' weight='normal' size='custom-15' color='blue-200'>{entry.value}</Typography>
        </div>
      ))}
    </div>
  );
