  ...
  return (
    <div className="flex items-center gap-4">
      ...
      <div className="flex flex-1 flex-col gap-1">
        <Typography type="body" size="md" weight='medium' color='gray-1000'>{transaction.description}</Typography>
        <Typography type="body" size="custom-15" weight='normal' color='blue-200'>{toLocaleDate(transaction.date)}</Typography>
      </div>
      <div className={getTextColor(transaction.type)}>
        <Typography type="body" size="md" weight='medium' color='inherit'>{toCurrency(transaction.amount)}</Typography>
      </div>
    </div>
  )
}